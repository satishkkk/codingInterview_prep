...
public class A4BitonicSubsequence {
    ...
    public static int longestBitonicSubarray(int[] arr){
        ...
        //generate left subs sequence
        for(int i=1;i<arr.length;i++){
            int candidate =0;
            for(int j=i-1;j>=0;j--){
                if(arr[j] < arr[i]){
                    candidate = Math.max(candidate,left[j]);
                }
            }
            left[i]=candidate+1;
        }

        //generate right subs sequence
        for(int i=arr.length-2;i>=0;i--){
            int candidate =0;
            for(int j=i+1;j<arr.length;j++){
                if(arr[j] <arr[i]){
                    candidate = Math.max(candidate,right[j]);
                }
            }
            right[i]=candidate+1;
        }
