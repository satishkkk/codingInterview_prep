...
public class WarmReception {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int[] inTime = new int[input];
        int[] outTime = new int[input];
        String[] inTimeString = br.readLine().split("\\s+");
        String[] outTimeString = br.readLine().split("\\s+");
        for(int i=0;i<input;i++){
            inTime[i] = Integer.parseInt(inTimeString[i]);
            outTime[i] = Integer.parseInt(outTimeString[i]);
        }
